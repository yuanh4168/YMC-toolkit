...
public:
    ...
    virtual ~Control() {}
    virtual void Draw(Gdiplus::Graphics* g) { (void)g; }
...
namespace eui {
    ...
    inline LRESULT CALLBACK InternalWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
        ...
        if (!gs.gdiplusInit) return DefWindowProcW(hwnd, msg, wParam, lParam);
...
inline int EasyUI::Run(const eui::Application& app) {
    auto& gs = eui::detail::GS();
    WNDCLASSEXW wc = { sizeof(WNDCLASSEXW), CS_HREDRAW|CS_VREDRAW, eui::InternalWndProc, 0,0,
    ...
    HWND hwnd = CreateWindowExW(0, L"EasyUIWindow", app.title.c_str(),
